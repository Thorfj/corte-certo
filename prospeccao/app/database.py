"""Camada de persistencia SQLite para a prospeccao de barbearias (Corte Certo)."""
...
DB_PATH = Path(__file__).resolve().parent.parent / "data" / "prospeccao_barbearias.db"
...
class Prospect:
    ...
    cidade: str = ""
    google_maps_url: str = ""
    tag_qualificacao: str = "🟢 Alta Oportunidade - Sem Agendamento Online"
    ...
    if row is None:
        conn.execute(
            """INSERT INTO prospects (
                ...
                instagram, endereco, bairro, cidade, google_maps_url, tag_qualificacao,
            ...
            (
                ...
                prospect.bairro, prospect.cidade, prospect.google_maps_url, prospect.tag_qualificacao,
...
def fetch_filtered(conn: sqlite3.Connection, filtro: str) -> list[sqlite3.Row]:
    ...
    if filtro == "Apenas Alta Oportunidade (sem agendamento online)":
        return conn.execute(
