"""Scraping de perfis de barbearias no Google Maps via Playwright."""
...
from app.qualification import analisar_texto, combinar_com_plataforma_url
...
def _montar_query(tipo: str, bairro: str, cidade: str) -> str:
    if bairro:
        return f"{tipo} em {bairro}, {cidade}"
    return f"{tipo} em {cidade}"
...
def buscar_perfis(
    ...
    """Abre o Google Maps, busca o tipo de estabelecimento no bairro/cidade e itera pelos primeiros N resultados."""
    query = _montar_query(tipo, bairro, cidade)
    ...
    with sync_playwright() as p:
        ...
        for i, link in enumerate(links, start=1):
            ...
            try:
                listing = _extrair_perfil(page, link, tipo)
                if listing:
                    yield listing
            except Exception:
                continue
...
def _extrair_perfil(page: Page, url: str, tipo: str) -> Optional[RawListing]:
    ...
    return RawListing(
        ...
        site=site,
        url_perfil=url,
        categoria_encontrada=tipo,
        descricao=descricao,
        avaliacoes_texto=avaliacoes_texto,
    )


def montar_prospect(listing: RawListing, tipo: str, bairro: str, cidade: str) -> Prospect:
    ...
    tag, encontradas = combinar_com_plataforma_url(tag, encontradas, listing.site)
